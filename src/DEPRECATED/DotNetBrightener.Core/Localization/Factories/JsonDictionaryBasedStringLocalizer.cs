...
public class JsonDictionaryBasedStringLocalizer : IStringLocalizer
{
    ...
    {
        var currentRequest = _httpContextAccessor.HttpContext.RequestServices.GetService<IRequestWorkContext>();

        var culture = currentRequest?.RetrieveState<CultureInfo>(CoreConstants.CurrentCultureRequestKey)
