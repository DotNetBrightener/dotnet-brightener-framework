...
internal class InMemoryEventProcessor<T> where T : IEventMessage
{
    ...
    public async Task ProcessEventMessage(T eventMessage)
    {
        using (IServiceScope processingScope = _serviceScopeFactory.CreateScope())
        {
            ...
            {
                var messageTypeGeneric = messageType.GetGenericTypeDefinition();

                var genericEventHandler = processingScope!.ServiceProvider
                                                       .GetService<IGenericEventHandler>();

                if (genericEventHandler is null)
                    return;

                await HandleGenericEventMessage(genericEventHandler, eventMessage);
    ...
    private async Task<bool> HandleGenericEventMessage(IGenericEventHandler x, T eventMessage)
    {
        try
        {
            var handleEventMethod = x.GetMethodWithName(nameof(IGenericEventHandler.HandleEvent));

            if (handleEventMethod == null)
                return false;

            handleEventMethod = handleEventMethod.MakeGenericMethod(eventMessage.GetType());
