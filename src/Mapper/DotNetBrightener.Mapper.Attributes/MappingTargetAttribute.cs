...
public sealed class MappingTargetAttribute<TSource> : Attribute
{
    /// <summary>
    ...
    ///     When specified, only these properties will be included in the target.
    ...
    ///     Must match the signature defined in IMappingConfiguration&lt;TSource, TTarget&gt;.
    /// </summary>
    /// <remarks>
    ///     The type must define a static method with one of the following signatures:
    ///     <c>
    ///         public static void Map(TSource source, TTarget target)</c> for mutable properties, or
    ///         <c>
    ///             public static TTarget Map(TSource source, TTarget target)</c> for init-only properties and records.
    ...
    /// <summary>
    ///     Whether to generate a method to map from the target type back to the source type.
    ...
    /// <summary>
    ///     If true, all non-nullable properties from the source type will be made nullable in the generated target.
    ...
    /// <summary>
    ///     An array of nested mapping types that represent nested objects within the source type.
    ///     When specified, the generator will automatically map properties of the source type to these nested mapping types.
    /// </summary>
    public Type[]? NestedTargetTypes { get; set; }

    /// <summary>
    ///     When true, copies attributes from the source type members to the generated target members.
    ...
    /// <summary>
    ///     The maximum depth for nested target recursion. When set to a positive value, the generator will
    ///     limit how deep nested targets can be instantiated to prevent stack overflow with circular references.
    ...
    /// <summary>
    ///     When true, the generator will track object references during target construction to prevent
    ...
    /// <summary>
    ///     An array of flattened types that can be generated from this target's collection properties.
    ...
    ///
    ///             // Define the targets
    ///             [MappingTarget&lt;Extended&gt;]
    ///             public partial class ExtendedDto;
    ///
    ///             [MappingTarget&lt;Data&gt;(NestedTargetTypes = [typeof(ExtendedDto)], FlattenTo = [typeof(DataFlattened)])]
    ...
    ///     Must implement IBeforeMapConfiguration&lt;TSource, TTarget&gt; with a static BeforeMap method.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         The type must define a static method with the signature:
    ///         <c>
    ///             public static void BeforeMap(TSource source, TTarget target)
    ///         </c>
    ///     </para>
    ///     <para>
    ...
    ///     Must implement IAfterMapConfiguration&lt;TSource, TTarget&gt; with a static AfterMap method.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         The type must define a static method with the signature:
    ///         <c>
    ///             public static void AfterMap(TSource source, TTarget target)
    ///         </c>
    ///     </para>
    ///     <para>
    ...
    /// <summary>
    ///     When true, generates a copy constructor that accepts another instance of the same target type
    ...
    ///     <c>
    ///         GetHashCode()</c>, and the <c>==</c> and <c>!=</c> operators.
    ...
    /// <summary>
    ///     Creates a new mapping target attribute for <typeparamref name="TSource"/> and excludes the specified members.
    /// </summary>
    /// <param name="exclude">
    ///     The names of the properties or fields to exclude.
    /// </param>
    public MappingTargetAttribute(params string[] exclude)
    {
