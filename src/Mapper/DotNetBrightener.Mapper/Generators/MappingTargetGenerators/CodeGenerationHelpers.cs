...
internal static class CodeGenerationHelpers
{
    ...
    /// </summary>
    public static HashSet<string> CollectNamespaces(MappableTargetModel model)
    {
        ...
        foreach (var member in model.Members)
        {
            ...
            // Skip nested targets and nested types - they will be handled by CollectStaticUsingTypes
            if (member.IsNestedTarget || member.IsNestedType)
            {
                continue;
            }
    ...
    /// </summary>
    public static HashSet<string> CollectStaticUsingTypes(MappableTargetModel model)
    {
        ...
        foreach (var member in model.Members)
        {
            if (member.IsNestedType && !member.IsNestedTarget)
            {
                ...
                if (lastDotIndex > 0)
                {
                    ...
                    if (!containingType.StartsWith("System.") && containingType != "System")
                    {
                        staticUsingTypes.Add(containingType);
                    }
                }
        ...
        // Check for nested targets that are actually nested classes (not just types in the same namespace)
        // Example: UserDetailResponse.UserAddressItem is a nested class
        // vs OrderItemTarget which would be a separate top-level type
        foreach (var member in model.Members)
        {
    ...
    /// </summary>
    public static string GetIndentation(MappableTargetModel model)
    {
