...
/// <summary>
///     Generates constructors for target types, including depth-aware constructors for circular reference prevention
///     and parameterless constructors.
/// </summary>
internal static class ConstructorGenerator
{
    /// <summary>
    ///     Generates the main constructor and related factory methods for the target type.
    /// </summary>
        ...
        bool hasRequiredProperties)
    {
        ...
        // Check if we have nested targets and depth tracking is needed
        bool hasNestedTargets = model.Members.Any(m => m.IsNestedTarget);
        // IMPORTANT: Generate depth-aware constructors whenever MaxDepth > 0 OR PreserveReferences is enabled
        // This ensures ALL targets (even those without nested targets) can be instantiated with depth tracking
        // when they are used as nested targets by other targets that have depth tracking enabled
    ...
    /// <summary>
    ///     Generates a parameterless constructor for the target type.
    /// </summary>
    public static void GenerateParameterlessConstructor(StringBuilder sb, MappableTargetModel model, bool isPositional)
    {
        sb.AppendLine();
        ...
        if (model.HasExistingPrimaryConstructor && model.IsRecord)
        {
            ...
            sb.AppendLine($"    // var instance = new {model.Name}(primaryConstructorParams) {{ /* initialize targeted properties */ }};");
            return;
        }
        ...
        bool hasCustomMapping)
    {
        ...
        if (!isPositional && !model.HasExistingPrimaryConstructor)
        {
            ...
            {
                // Regular mutable properties - initialize properly (including nested targets), then apply custom mapping
                // This ensures nested targets are instantiated before custom mapping logic runs
    ...
    ///     This is public to support nested targets across assemblies.
    /// </summary>
        ...
        bool hasRequiredProperties)
    {
        ...
        sb.AppendLine("    /// <remarks>");
        sb.AppendLine("    /// This constructor is public to support nested targets across different assemblies.");
        ...
        bool hasCustomMapping)
    {
        ...
        else if (hasCustomMapping)
        {
            ...
            // This ensures nested targets are properly instantiated with depth parameters
    ...
    private static void GenerateFromSourceFactoryMethod(StringBuilder sb, MappableTargetModel model, bool hasCustomMapping, bool needsDepthTracking)
    {
        sb.AppendLine();
        sb.AppendLine("    /// <summary>");
    ...
    private static void GenerateFactoryMethodForExistingPrimaryConstructor(StringBuilder sb, MappableTargetModel model, bool hasCustomMapping)
    {
        ...
        sb.AppendLine($"    /// <summary>");
        sb.AppendLine($"    /// Creates a new {model.Name} from the source with targeted properties initialized.");
        ...
        if (hasCustomMapping)
        {
            sb.AppendLine($"        // Custom mapping is configured for this target");
            sb.AppendLine($"        // The custom mapper should handle both the primary constructor and targeted properties");
        ...
        else
        {
            ...
            sb.AppendLine($"        // and initialize the targeted properties using object initializer syntax.");
