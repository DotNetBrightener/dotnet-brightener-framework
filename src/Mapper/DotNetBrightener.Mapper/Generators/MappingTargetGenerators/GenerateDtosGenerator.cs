...
{
    private const string GenerateDtosAttributeName = "DotNetBrightener.Mapper.GenerateDtosAttribute";
    // Keep for backward compatibility with obsolete attribute
    private const string GenerateAuditableDtosAttributeName = "DotNetBrightener.Mapper.GenerateAuditableDtosAttribute";

    // Diagnostic for generator internal errors
    private static readonly DiagnosticDescriptor GeneratorErrorRule = new(
                                                                          "DNB100",
    ...
    private static readonly HashSet<string> DefaultAuditFields = new(System.StringComparer.OrdinalIgnoreCase)
    {
        "CreatedDate",
        "UpdatedDate",
        "ModifiedDate",
    ...
    private static readonly HashSet<string> IdFieldPatterns = new(System.StringComparer.OrdinalIgnoreCase)
    {
        "Id"
    };
    ...
    private static void GenerateDtosForModel(SourceProductionContext context, GenerateDtosTargetModel model)
    {
        ...
        {
            var queryMembers = model.Members.Select(m => new MappableTargetMember(
                m.Name,
    ...
    /// </summary>
    private static ImmutableArray<MappableTargetMember> FilterMembers(
        ImmutableArray<MappableTargetMember> members,
    ...
    private static string GenerateDtoCode(GenerateDtosTargetModel model, string dtoName, ImmutableArray<MappableTargetMember> members, string purpose)
    {
    ...
    private static void GenerateDtoTypeDeclaration(StringBuilder sb, GenerateDtosTargetModel model, string dtoName, string sourceTypeName, string purpose)
    {
        ...
        // Add [MappingTarget] attribute to make it work with extension methods
        sb.AppendLine($"[DotNetBrightener.Mapper.MappingTarget<{model.SourceTypeName}>()]");

        sb.AppendLine($"public {keyword} {dtoName}");
        sb.AppendLine("{");
    }

    private static void GenerateDtoMembers(StringBuilder sb, ImmutableArray<MappableTargetMember> members)
    {
        foreach (var member in members)
        {
            if (member.Kind == TargetMemberKind.Property)
            {
    ...
    private static void GenerateDtoProperty(StringBuilder sb, MappableTargetMember member)
    {
    ...
    private static void GenerateDtoField(StringBuilder sb, MappableTargetMember member)
    {
        var fieldDef = "public";
    ...
    private static void GenerateDtoConstructors(StringBuilder sb, GenerateDtosTargetModel model, string dtoName, string sourceTypeName, ImmutableArray<MappableTargetMember> members, bool hasInitOnlyProperties, bool hasReadOnlyFields)
    {
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>");
    ...
    private static void GenerateDtoFromSourceFactory(StringBuilder sb, GenerateDtosTargetModel model, string dtoName, string sourceTypeName, ImmutableArray<MappableTargetMember> members, bool hasReadOnlyFields)
    {
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>");
    ...
    private static void GenerateDtoProjection(StringBuilder sb, GenerateDtosTargetModel model, string dtoName, string sourceTypeName, ImmutableArray<MappableTargetMember> members, bool hasInitOnlyProperties, bool hasReadOnlyFields)
    {
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>");
    ...
    private static void GenerateDtoToSource(StringBuilder sb, GenerateDtosTargetModel model, string dtoName, string sourceTypeName, ImmutableArray<MappableTargetMember> members)
    {
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>");
    ...
    private static string GeneratePatchDtoCode(GenerateDtosTargetModel model, string dtoName, ImmutableArray<MappableTargetMember> members)
    {
        var sb = new StringBuilder();
        var sourceTypeName = GetSimpleTypeName(model.SourceTypeName);
        ...
        foreach (var member in members)
        {
            if (member.Kind == TargetMemberKind.Property)
            {
                ...
                sb.AppendLine($"    public global::DotNetBrightener.Mapper.Optional<{member.TypeName}> {member.Name} {{ get; set; }}");
            }
        }
    ...
    private static void GeneratePatchApplyToMethod(StringBuilder sb, GenerateDtosTargetModel model, string dtoName, string sourceTypeName, ImmutableArray<MappableTargetMember> members)
    {
        sb.AppendLine();
        sb.AppendLine($"    /// <summary>");
        ...
        var updatableMembers = members.Where(m => !m.IsReadOnly && m.Kind == TargetMemberKind.Property).ToArray();
    ...
    private static void GenerateFileHeader(StringBuilder sb)
    {
        ...
        sb.AppendLine("//     This code was generated by the mapping target GenerateDtos source generator.");
