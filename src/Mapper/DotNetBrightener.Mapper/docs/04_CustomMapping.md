# Custom Mapping with IMappingConfiguration

`DotNetBrightener.Mapper.Mapping` supports custom mapping logic for advanced scenarios via multiple interfaces that handle different mapping requirements.
...
|-----------|---------|----------|
| `IMappingConfiguration<TSource, TTarget>` | Synchronous mapping | Fast, in-memory operations |
| `IMappingConfigurationAsync<TSource, TTarget>` | Asynchronous mapping | I/O operations, database calls, API calls |

### Instance Mappers (With Dependency Injection Support)
| Interface | Purpose | Use Case |
|-----------|---------|----------|
| `IMappingConfigurationInstance<TSource, TTarget>` | Synchronous mapping | Fast operations with injected services |
| `IMappingConfigurationAsyncInstance<TSource, TTarget>` | Asynchronous mapping | I/O operations with injected services |
...
## DotNetBrightener.Mapper.Mapping Extension Methods

The following extension methods are available for async mapping scenarios:

| Method | Description |
|--------|-------------|
| `ToTargetAsync<TSource, TTarget, TMapper>()` | Map single instance with explicit type parameters (compile-time) |
| `ToTargetAsync<TTarget, TMapper>()` | Map single instance with inferred source type (runtime reflection) |
| `ToTargetsAsync<TSource, TTarget, TMapper>()` | Map collection sequentially with explicit types |
| `ToTargetsAsync<TTarget, TMapper>()` | Map collection sequentially with inferred source type |
| `ToTargetsParallelAsync<TSource, TTarget, TMapper>()` | Map collection in parallel with explicit types |
| `ToTargetsParallelAsync<TTarget, TMapper>()` | Map collection in parallel with inferred source type |
...
public class UserMapConfig : IMappingConfiguration<User, UserDto>
{
...
### 2. Reference in the MappingTarget Attribute

```csharp
[MappingTarget<User>(Configuration = typeof(UserMapConfig))]
...
public class UserAsyncMapperWithDI : IMappingConfigurationAsyncInstance<User, UserDto>
{
...
public class UserController : ControllerBase
{
    ...
    public async Task<UserDto> GetUser(int id)
    {
        ...
        return await user.ToTargetAsync(_userMapper);
    }
    ...
    public async Task<List<UserDto>> GetUsers()
    {
        ...
        return await users.ToTargetsAsync(_userMapper);
    }
}
```
...
public class UserAsyncMapper : IMappingConfigurationAsync<User, UserDto>
{
...
var userDto = await user.ToTargetAsync<UserDto, UserAsyncMapper>();

// Collection async mapping (sequential, static)
var userDtos = await users.ToTargetsAsync<UserDto, UserAsyncMapper>();

// Collection async mapping (parallel for better performance, static)
var userDtosParallel = await users.ToTargetsParallelAsync<UserDto, UserAsyncMapper>(
    maxDegreeOfParallelism: 4);
```

## Combining Sync and Async Mapping

The hybrid interfaces are obsolete. Prefer implementing the sync and async interfaces directly on the same mapper type.

### Static Mapper with Sync and Async Logic

```csharp
public class UserMapper :
    IMappingConfiguration<User, UserDto>,
...
// Usage
var userDto = await user.ToTargetAsync<UserDto, UserMapper>();
```

### Instance Mapper with Sync and Async Logic

```csharp
public class UserMapperWithDI :
    IMappingConfigurationInstance<User, UserDto>,
    IMappingConfigurationAsyncInstance<User, UserDto>
{
    private readonly IProfilePictureService _profilePictureService;
    private readonly IReputationService _reputationService;

    public UserMapperWithDI(IProfilePictureService profilePictureService, IReputationService reputationService)
    {
        _profilePictureService = profilePictureService;
        _reputationService = reputationService;
    }
...
// Usage with DI
var userDto = await user.ToTargetAsync(userMapperWithDI);
```
...
var userDto1 = await user.ToTargetAsync<UserDto, UserAsyncMapper>();
...
var userDto2 = await user.ToTargetAsync(mapper);
```
...
```csharp
public class SafeUserAsyncMapperWithDI : IMappingConfigurationAsyncInstance<User, UserDto>
{
    private readonly IProfilePictureService _profilePictureService;
    private readonly IReputationService _reputationService;
...
|----------|----------------------|---------|
| Simple property transformations | `IMappingConfiguration` (static) | Zero overhead, compile-time optimized |
| Complex transformations needing services | `IMappingConfigurationInstance` | Full DI support, easier testing |
| Database lookups | `IMappingConfigurationAsyncInstance` | Proper async/await with injected DbContext |
| API calls | `IMappingConfigurationAsyncInstance` | Non-blocking I/O with injected HttpClient |
| Mixed fast/slow operations | `IMappingConfigurationInstance` + `IMappingConfigurationAsyncInstance` | Best of both worlds with DI |
...
var results = await items.ToTargetsAsync(mapper);

// For large collections with I/O operations and DI
var results = await items.ToTargetsParallelAsync(mapper, maxDegreeOfParallelism: Environment.ProcessorCount);

// For database-heavy operations (avoid overwhelming the database)
var results = await items.ToTargetsParallelAsync(mapper, maxDegreeOfParallelism: 2);
```
...
var result = await user.ToTargetAsync<UserDto, ExistingAsyncMapper>();
```
...
public class ExistingAsyncMapperWithDI : IMappingConfigurationAsyncInstance<User, UserDto>
{
...
var result = await user.ToTargetAsync(mapper);
```
