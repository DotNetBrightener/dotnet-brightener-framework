...
This page shows concrete examples of what `DotNetBrightener.Mapper` generates for different scenarios. These examples are based on the actual test suite and reflect the current public API.
...
## 1. Basic Class Target (Exclude Mode)
...
[MappingTarget<User>(Include = [nameof(User.FirstName), nameof(User.LastName), nameof(User.Email)], GenerateToSource = true)]
...
public class UserDtoMapper : IMappingConfiguration<User, UserDto>
{
...
[MappingTarget<User>("Password", "CreatedAt", Configuration = typeof(UserDtoMapper))]
public partial class UserDto
{
    public string FullName { get; set; } = string.Empty;
    public int Age { get; set; }
...
## 4. Record Targets with Positional Parameters
...
[MappingTarget<UserForNestedFacet>(Include = [
...
{
    [MappingTarget<UserAddressForNestedFacet>(Include = [
...
// UserDetailResponse uses the nested target type
public partial class UserDetailResponse
{
    public int Id { get; set; }
    public string Name { get; set; }
...
**Key Points:**
- Nested target types create nested DTOs
- Depth tracking prevents infinite recursion for circular references
- Nested target types are specified via `NestedTargetTypes` parameter

---

## 11. Generation Control Flags

`DotNetBrightener.Mapper` provides fine-grained control over what gets generated:

```csharp
[MappingTarget<Source>(GenerateConstructor = false,              // Skip source constructor
...
For a basic target type, `DotNetBrightener.Mapper` generates:
...
If you're upgrading from an earlier version of the library, here are the major changes to generated code:
