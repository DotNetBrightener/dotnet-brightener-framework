# Asynchronous Mapping with DotNetBrightener.Mapper.Mapping

This guide covers advanced asynchronous mapping scenarios with DotNetBrightener.Mapper.Mapping, including real-world examples and best practices.

## Overview

DotNetBrightener.Mapper.Mapping provides comprehensive async mapping support for scenarios where your mapping logic needs to perform I/O operations like:
...
### IMappingConfigurationAsync<TSource, TTarget>
...
The hybrid interfaces are obsolete. Prefer implementing the sync and async interfaces directly on the same mapper type, or their `Instance` counterparts for dependency injection scenarios.
...
public class UserProfileAsyncMapper : IMappingConfigurationAsync<User, UserProfileDto>
{
...
var userProfile = await user.ToTargetAsync<UserProfileDto, UserProfileAsyncMapper>();
```
...
public class ProductMapper :
    IMappingConfiguration<Product, ProductDto>,
    IMappingConfigurationAsync<Product, ProductDto>
{
...
var productDto = await product.ToTargetAsync<ProductDto, ProductMapper>();

// Multiple products with parallel processing
var products = await dbContext.Products.Take(20).ToListAsync();
var productDtos = await products.ToTargetsParallelAsync<ProductDto, ProductMapper>(
...
var results = await items.ToTargetsAsync<Target, AsyncMapper>();
```

### Parallel Processing
```csharp
// Best for: Large collections, independent operations
var results = await items.ToTargetsParallelAsync<Target, AsyncMapper>(
    ...
    int batchSize = 50)
{
    ...
    {
        var batchResults = await batch.ToTargetsParallelAsync<ProductDto, ProductMapper>(
            maxDegreeOfParallelism: 4);
...
```csharp
public class ResilientUserMapper : IMappingConfigurationAsync<User, UserDto>
{
...
```csharp
public class CircuitBreakerUserMapper : IMappingConfigurationAsync<User, UserDto>
{
...
```csharp
public class CachedAsyncMapper : IMappingConfigurationAsync<User, UserDto>
{
    private static readonly IMemoryCache _cache = new MemoryCache(new MemoryCacheOptions
    {
        SizeLimit = 1000
...
var dbResults = await items.ToTargetsParallelAsync<Target, DbMapper>(maxDegreeOfParallelism: 2);

// For HTTP APIs: Medium concurrency
var apiResults = await items.ToTargetsParallelAsync<Target, ApiMapper>(maxDegreeOfParallelism: 4);

// For CPU-bound operations: High concurrency
var cpuResults = await items.ToTargetsParallelAsync<Target, CpuMapper>(maxDegreeOfParallelism: Environment.ProcessorCount);
```
...
This guide reflects the current async mapping APIs in `DotNetBrightener.Mapper.Mapping`, including the preferred static and instance async mapper patterns.
