# Expression Mapping with DotNetBrightener.Mapper.Mapping

Transform business logic expressions between entities and DTOs with `DotNetBrightener.Mapper.Mapping`. This library enables you to define business rules, filters, and selectors once for your entities and seamlessly use them with your generated target DTOs.
...
Expression<Func<UserDto, bool>> activeDtoUsers = activeUsers.MapToTarget<UserDto>();
...
Expression<Func<UserDto, string>> dtoSortByLastName = sortByLastName.MapToTarget<UserDto, string>();
...
var dtoProjection = complexProjection.MapToTargetGeneric<UserDto>();
```
...
// Combine with AND
var validUserFilter = MappingExpressionExtensions.CombineWithAnd(isAdult, isActive, hasValidEmail);

// Combine with OR
var flexibleFilter = MappingExpressionExtensions.CombineWithOr(isAdult, hasValidEmail);
...
var inactiveDtoUsers = inactiveUsers.MapToTarget<UserDto>();
```
...
public class UserService
{
    ...
    {
        var dtoFilter = _activeUsersFilter.MapToTarget<UserDto>();
        return dtos.Where(dtoFilter.Compile());
    }
    
    public IEnumerable<UserDto> SortDtosByName(IEnumerable<UserDto> dtos)
    {
        var dtoSelector = _displayNameSelector.MapToTarget<UserDto, string>();
...
var entityFilter = MappingExpressionExtensions.CombineWithAnd(filters.ToArray());
        var dtoFilter = entityFilter.MapToTarget<UserDto>();
...
public class UsersController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<UserDto>>> GetDtos()
    {
        ...
        var dtos = users.Select(u => u.ToTarget<UserDto>()).ToList();
        
        var dtoFilter = _publicUsersFilter.MapToTarget<UserDto>();
...
var dtoFilter = recentOrders.MapToTarget<OrderDto>();
```
...
var dtoFilter = emailDomainFilter.MapToTarget<UserDto>();
```
...
```csharp
var eligibleActiveUser = MappingExpressionExtensions.CombineWithAnd(
...
public class UserFilterCache
{
    private static readonly Lazy<Expression<Func<UserDto, bool>>> _activeDtoFilter = 
        new(() => BusinessRules.ActiveUser.MapToTarget<UserDto>());
...
var dtoFilter = ageFilter.MapToTarget<UserDto>(); // May throw if Age doesn't exist
```
...
## Integration with Other Mapping Packages

Expression mapping works seamlessly with:

- **DotNetBrightener.Mapper**: All generated `MappingTarget` types are supported
- **DotNetBrightener.Mapper.Mapping**: Use with LINQ extension methods
- **DotNetBrightener.Mapper.Mapping**: Combine with custom mapping configurations
- **DotNetBrightener.Mapper.Mapping.EFCore**: Transform expressions before EF Core queries
