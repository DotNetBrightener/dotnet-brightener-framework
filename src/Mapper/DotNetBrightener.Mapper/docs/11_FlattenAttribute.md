...
Flattening transforms a hierarchical object structure into a flat structure with all nested properties as top-level properties. Instead of manually writing DTOs with properties like `AddressStreet`, `AddressCity`, and so on, `DotNetBrightener.Mapper` generates them automatically by traversing your domain model.
...
**Warning**: `LeafOnly` can cause name collisions if multiple nested objects have properties with the same name. `DotNetBrightener.Mapper` will automatically add numeric suffixes such as `Name2` and `Name3` to resolve collisions.
...
Even with `MaxDepth = 0` (unlimited), `DotNetBrightener.Mapper` enforces a safety limit of 10 levels to prevent stack overflow from circular references or extremely deep hierarchies.

## What Gets Flattened?

`DotNetBrightener.Mapper` automatically determines which types should be flattened as "leaf" properties and which should be recursed into:
...
**Solution**: `DotNetBrightener.Mapper` has built-in protection. It tracks visited types, prevents infinite recursion, and enforces a safety limit of 10 levels.

## Comparison: Flatten vs MappingTarget with NestedTargetTypes

| Feature | `[Flatten]` | `[MappingTarget]` with `NestedTargetTypes` |
|---------|-------------|-------------------------------|
...
| **Setup** | Single attribute, automatic | Requires defining each nested target type |
| **Flexibility** | Less (automatic) | More (explicit control) |

## FlattenTo Property (Unpack Collections to Rows)

While the `[Flatten]` attribute flattens nested objects into a single DTO, the `FlattenTo` property on the `[MappingTarget]` attribute unpacks collection properties into multiple rows. This is useful for reports, exports, and scenarios where you need to denormalize a parent-child relationship.

### What is FlattenTo?

`FlattenTo` transforms a target DTO with a collection property into multiple rows, combining the parent properties with each collection item:
...
// Target type for the collection item
[MappingTarget<ExtendedEntity>]
public partial class ExtendedFacet;

// Target type for the parent with FlattenTo
[MappingTarget<DataEntity>(NestedTargetTypes = [typeof(ExtendedFacet)],
...
var target = new DataFacet(entity);
var rows = target.FlattenTo();
...
| **Input** | Target DTO with collection | Entity with nested objects |
