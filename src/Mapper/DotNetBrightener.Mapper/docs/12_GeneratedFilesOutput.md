...
By default, the Roslyn source generators used by `DotNetBrightener.Mapper` output generated files to the `obj/Generated/` folder, which is hidden from Solution Explorer. However, you can configure where generated files are written using standard MSBuild properties.
...
This is what the project's test suites use internally.
...
│   ├── DotNetBrightener.Mapper.Generators.TargetGenerator/
│   │   ├── UserDto.g.cs
│   │   ├── ProductDto.g.cs
│   │   └── ...
│   ├── DotNetBrightener.Mapper.Generators.FlattenGenerator/
│   │   └── ...
│   └── DotNetBrightener.Mapper.Generators.GenerateDtosGenerator/
│       └── ...
└── OtherGenerator/
    └── ...
```
...
2. **Learning**: Understand how the generators produce code by inspecting the output
