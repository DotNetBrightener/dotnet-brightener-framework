...
The `[Wrapper]` attribute generates wrapper classes that **delegate** to a source object instance, creating a reference-based facade pattern. Unlike `[MappingTarget]` which creates independent value copies, wrappers maintain a reference to the source object, so changes to wrapper properties affect the underlying source.

## When to Use Wrapper vs MappingTarget

| Use Case | Use Wrapper | Use MappingTarget |
|----------|-------------|-----------|
...
- Don't mix `MappingTarget` and `Wrapper` for the same source type in nested scenarios
...
- Combine with `MappingTarget` when you need both patterns for different purposes

### Don't

- Don't use wrappers for **DTOs** or **data transfer** (use `MappingTarget` instead)
- Don't use wrappers for **EF Core query projections** (use `MappingTarget` instead)
- Don't use wrappers for **serialization** (use `MappingTarget` instead)
- Don't wrap generated target DTOs. Both should target the same source type.
...
## Comparison: Wrapper vs MappingTarget

```csharp
public class User { public string Name { get; set; } }

// MappingTarget: Creates independent copy
[MappingTarget<User>]
...
- [MappingTarget Attribute](03_AttributeReference.md) - For value-copying behavior
- [Advanced Scenarios](06_AdvancedScenarios.md) - Complex patterns
- [Quick Start](02_QuickStart.md) - Getting started with mapping target generation
