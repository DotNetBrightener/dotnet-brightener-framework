...
The `ConvertEnumsTo` property on the `[MappingTarget]` attribute allows you to automatically convert all enum properties from a source type into a different representation (`string` or `int`) in the generated target. This is useful for API DTOs, serialization scenarios, and database storage where you need enum values as strings or integers rather than their enum types.
...
// All enum properties become string in the generated target
...
```csharp
// All enum properties become int in the generated target
...
### ToSource (Target -> Source) - Reverse Mapping
...
```csharp
[MappingTarget<User>(ConvertEnumsTo = typeof(string), NullableProperties = true)]
...
```csharp
[MappingTarget<User>(Include = [nameof(User.Id), nameof(User.Name), nameof(User.Status)],
...
1. **All enum properties are converted**: The setting applies to every enum property in the source type. If you need mixed behavior (some enums converted, some not), consider using separate target DTOs or a custom mapping configuration.
